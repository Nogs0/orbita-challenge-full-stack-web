<template>
  <v-sheet border rounded>
    ...
    >
      <template v-slot:top>
        <header-table
          table-name="Seus Alunos"
          icon="mdi-account-school"
          search-fields="Nome, RA ou CPF"
      ...
      <template v-slot:item.actions="{ item }">
        <div class="d-flex ga-2 justify-center">
          <v-icon
            ...
            size="small"
            title="Matrículas"
            @click="openSetCoursesDialog(item.id, item.name)"
          ></v-icon>
          <v-icon
            color="medium-emphasis"
            icon="mdi-pencil"
      ...
      <v-form ref="formCreateOrEditStudent" v-model="isFormValid">
        <v-container>
          ...
          <v-text-field
            ...
            label="Email *"
            maxlength="128"
            :rules="[rules.required, rules.maxLength(128), rules.email]"
          ></v-text-field>

          <v-text-field
            v-if="isEditing"
            v-model="formModel.ra"
            label="RA *"
            :rules="[rules.required, rules.maxLength(14)]"
            :disabled="isEditing"
          ></v-text-field>

          <v-text-field
            v-model="formModel.cpf"
            label="CPF *"
            :rules="[rules.required, rules.maxLength(14)]"
            :disabled="isEditing"
          ></v-text-field>
  ...
  <enrollment-modal
    :show="setCoursesDialog"
    :student-id="selectedStudentId"
    :student-name="selectedStudentName"
    @close="closeSetCoursesDialog()"
    @saved="closeSetCoursesDialog()"
  ></enrollment-modal>
